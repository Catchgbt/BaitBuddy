...
import { BUDDIES, normalizeBuddy, normalizeNavigation } from './buddyPreferences';
...
export function BuddyPreferencesProvider({ children }) {
  ...
  const value = useMemo(() => ({ userId, buddy, activeBuddy: BUDDIES[buddy.avatarId], navigation, saving, canSave: !!userId,
  ...
  }), [buddy, navigation, saving, userId, savePreferences]);
...
export function useBuddyPreferences() {
  ...
  return context || { buddy: normalizeBuddy(), activeBuddy: BUDDIES.female_default, navigation: normalizeNavigation(), saving: false, canSave: false };
}
