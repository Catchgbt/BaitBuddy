...
- **Frontend**: verteilt über mehrere Stellen (es gibt **kein** `src/components/KiBuddy/`-Verzeichnis):
  ...
  - `src/lib/buddyGreetings.js` — Start-Begrüßungs-Generator: einmal pro App-Sitzung (sessionStorage-Flag `bb_buddy_greeted`) begrüßt der Buddy per Sprechblase + TTS, variiert nach Tageszeit/Stimmung/Event-Status (Anti-Wiederholung via localStorage). Ausgelöst im `AIBuddyWidgetStub` (der beim App-Start gemountet ist); das volle Widget und `KiBuddyBeta` nutzen denselben Generator.
